...
list<PolicyItem *> perform_regression(const SearchEngine::Plan &plan, vector<pair<int, int> > goal, int distance, bool create_goal) {
    ...
    /* Since g_initial_state() returns a complete state, s is initially a complete state, but of PartialState type */
    PartialState *s = new PartialState(g_initial_state());
        
    vector<PartialState *> states;
    states.push_back(s);

    /* Progress through the plan and add all the states (as PartialStates) to the states vector */
    ...
    if (g_fullstate) {
        ...
        /* Create the partial goal state (assign values only to states defined by the goal) */
        for (int i = 0; i < goal.size(); i++) {
            (*s)[goal[i].first] = int(goal[i].second);
        }

        /* Create an empty regression step (regress 0 steps from the goal) */
        PartialState *curr_reg_state = s;
        reg_steps.push_back(new RegressionStep(s, distance));
        
        for (int i = plan.size() - 1; i >= 0; i--) {
            /* Regress from curr_reg_state through the action at plan[i] to the context at states[i] */
            curr_reg_state = new PartialState(*curr_reg_state, *plan[i], false, states[i]);
            /* Create a RegressionStep object to this regressed state via plan[i] action at an increased distance
             * from the goal */
            reg_steps.push_back(new RegressionStep(*plan[i], curr_reg_state, ++distance));
        }
    }
    ...
    /* TODO: Figure out what is this used for and how does it work (and is it really not necessary) */
    // Strengthen all of the steps so they don't fire a forbidden state-action
    //  pair at some point. The strengthening is sufficient but not necessary
