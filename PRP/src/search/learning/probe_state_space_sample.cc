...
void ProbeStateSpaceSample::send_probe(int depth_limit) {
    ...
    // TO_DO for now we use the global state registry for all states (even temporary states like this): see issue386
    ...
    for (int i = 0; (i < depth_limit) && (samp.size() < min_training_set_size); i++) {
        ...
        for (int op_num = 0; op_num < applicable_ops.size(); op_num++) {
            ...
            // TO_DO for now, we only generate registered successors. This is a temporary state that
