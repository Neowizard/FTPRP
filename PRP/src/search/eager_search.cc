...
void EagerSearch::initialize() {
    //TO_DO children classes should output which kind of search
...
int EagerSearch::step() {
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        if (succ_node.is_new()) {
            ...
            //TO_DO:CR - add an ID to each state, and then we can use a vector to save per-state information
            ...
            if (reopen_closed_nodes) {
                //TO_DO:CR - test if we should add a reevaluate flag and if it helps
                // if we reopen closed nodes, do that
                if (succ_node.is_closed()) {
                    /* TO_DO: Verify that the heuristic is inconsistent.
                     ...
                     * use a plain assert. */
                    //TO_DO:CR - add a consistent flag to heuristics, and add an assert here based on it
                    search_progress.inc_reopened();
                }
                succ_node.reopen(node, op);
                heuristics[0]->set_evaluator_value(succ_node.get_h());
                // TO_DO: this appears fishy to me. Why is here only heuristic[0]
...
pair<SearchNode, bool> EagerSearch::fetch_next_node() {
    /* TO_DO: The bulk of this code deals with multi-path dependence,
    ...
    while (true) {
        ...
        // TO_DO is there a way we can avoid creating the state here and then
