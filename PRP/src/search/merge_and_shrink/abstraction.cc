...
// TO_DO: move that type somewhere else?
#include "shrink_fh.h"

#include "../globals.h"
#include "../operator.h"
#include "../option_parser.h" // TO_DO: Should be removable later.
...
//  TO_DO: We define infinity in more than a few places right now (=>
//        grep for it). It should only be defined once.
static const int infinity = numeric_limits<int>::max();

inline int get_op_index(const Operator *op) {
    /* TO_DO: The op_index computation is duplicated from
...
void Abstraction::compute_distances() {
    ...
    if (unreachable_count || irrelevant_count) {
        ...
        /* TO_DO: Create a dedicated shrinking strategy from scratch,
           e.g. a bucket-based one that simply generates one good and
           one bad bucket? */

        // TO_DO/HACK: The way this is created is of course unspeakably
...
void Abstraction::compute_init_distances_general_cost() {
    ...
    // TO_DO: Reuse the same queue for multiple computations to save speed?
