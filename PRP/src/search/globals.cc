...
// TO_DO: This needs a proper type and should be moved to a separate
...
int calculate_plan_cost(const vector<const Operator *> &plan) {
    // TO_DO: Refactor: this is only used by save_plan (see below)
...
void save_plan(const vector<const Operator *> &plan, int iter) {
    // TO_DO: Refactor: this is only used by the SearchEngine classes
...
void dump_everything() {
    ...
    // TO_DO: Dump the actual fact names.
