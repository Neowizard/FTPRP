...
class CommandLine:
	...
	def _tokenize(self, cmd_string=''):
		...
		#TO_DO protect quotes contents -- Don't use it, pass cmdline as list
...
class ConfigMaster:
	...
	def sanity(self, config, gui=0):
		...
		if not gui:
			...
			# TO_DO#1: It seems that this checking is never reached
...
class ConfigLines:
	...
	def parse_line(self, line='', keyname='', target=''):
		...
		# XXX TO_DO <value>\S.+?  requires TWO chars, breaks %!include:a
		cfgregex  = re.compile("""
...
class TitleMaster:
	...
	def _set_label(self):
		...
		#TO_DO how to translate safely accented chars to plain?
...
#TO_DO check all this table mess
# Trata linhas TABLE, com as prop do parse_row
# O meTO_DO table() do BLOCK xunxa e troca as celulas pelas parseadas
class TableMaster:
...
def doHeader(headers, config):
	...
	#TO_DO loop by template line > key
	for key in head_data.keys():
...
# TO_DO man: where - really needs to be escaped?
...
def parse_deflist_term(line):
	...
	#TO_DO tex: escape ] on term. \], \rbrack{} and \verb!]! don't work :(
	return term
...
def process_source_file(file_='', noconf=0, contents=[]):
	...
	if not noconf:
		...
		#TO_DO pick a better name than --show-config-value
...
def convert_this_files(configs):
	...
	for myconf,doc in configs:                 # multifile support
		...
		#TO_DO escape line before?
		#TO_DO see exceptions by tex and mgp
...
def get_include_contents(file_, path=''):
	...
	if id_ == 't2t':
		...
		#TO_DO fix images relative path if file has a path, ie.: chapter1/index.t2t (wait until tree parsing)
		#TO_DO for the images path fix, also respect outfile path, if different from infile (wait until tree parsing)
...
def convert(bodylines, config, firstlinenr=1):
	...
	while lineref < len(bodylines):
		...
		if regex['special'].search(line):
			...
			if key == 'include':
				...
				if inctype != 't2t':
					...
					#TO_DO include maxdepth limit
					bodylines = bodylines[:lineref] + inclines + bodylines[lineref:]
					#TO_DO fix path if include@include
					# Remove %!include call
					if CONF['dump-source']:
						dump_source.pop()
		...
		#TO_DO escape undesired format inside table
		#TO_DO add pm6 target
		if regex['table'].search(line):
			...
			# TO_DO isolate unmask+inlines parsing to use here
			line = SEPARATOR.join(tablerow['cells'])
...
class Gui:
	...
	def askfile(self):
		...
		if newfile:
			...
			#TO_DO how to make a refresh without killing it?
			self.root.destroy()
...
def exec_command_line(user_cmdline=[]):
	...
	# Console mode rocks forever!
		...
		# TO_DO#1: this checking should be only in ConfigMaster.sanity()
		if not infiles:
			Error(_('Missing input file (try --help)') + '\n\n' +
